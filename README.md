...
![How Kafka Internally Works — Leader, Followers, ISR & KRaft Quorum](kafka-internals.png)
...
As an architect, I don't argue with the design docs — I design *with* them. When the official documentation tells me that Kafka tolerates **f failures with f+1 replicas** under the ISR model, and that `min.insync.replicas` exists precisely to prevent acknowledged messages from evaporating, then my job is simple: pick node counts and configs that honor the math. That's exactly what the rest of this article does.
